...
class laporan_pengiriman_barang extends MY_Controller 
{
	private $data = array();
	...
	{
		parent::__construct();
		$this->data['active_menu'] = 'laporan_pengiriman_barang';
		$this->data['title'] = 'Laporan Pengiriman Barang';
		$this->data['index'] = 'laporan_pengiriman_barang';
	...
	public function index()
	{
		...
		if ($this->form_validation->run()===false) {
			$this->data['action'] = site_url('laporan_pengiriman_barang');
			...
			$pdf->Ln(4);
			$pdf->Cell(0,5,'Laporan Pengiriman Barang',0,0,'L');
			$pdf->Ln(4);
			...
			foreach ($result as $row) {
				...
				$pdf->Cell(10,5,':',0,0,'C');
				$pdf->Cell(0,5,format_dmy($row->tanggal),0,0,'L');
				$pdf->Ln(5);
