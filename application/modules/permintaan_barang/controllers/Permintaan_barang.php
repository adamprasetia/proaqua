...
class Permintaan_barang extends MY_Controller 
{
	...
	public function __construct()
	{
		...
		$this->data['index'] = 'permintaan_barang';
	...
	public function index()
	{
		...
		foreach($result as $r){
			$this->table->add_row(
				...
				$r->status_name,
				($r->status==1?anchor($this->data['index'].'/edit/'.$r->id.get_query_string(),$this->lang->line('edit')).'&nbsp;|':'')
				."&nbsp;".anchor($this->data['index'].'/view/'.$r->id.get_query_string(),'View')
				.($r->status==1?"&nbsp;|&nbsp;".anchor($this->data['index'].'/delete/'.$r->id.get_query_string(),$this->lang->line('delete'),array('onclick'=>"return confirm('".$this->lang->line('confirm')."')")):'')
			);
		}
