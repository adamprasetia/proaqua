...
class laporan_permintaan_barang extends MY_Controller 
{
	private $data = array();
	...
	{
		parent::__construct();
		$this->data['active_menu'] = 'laporan_permintaan_barang';
		$this->data['title'] = 'Laporan Permintaan Barang';
		$this->data['index'] = 'laporan_permintaan_barang';
	...
	public function index()
	{
		...
		if ($this->form_validation->run()===false) {
			$this->data['action'] = site_url('laporan_permintaan_barang');
			...
			$pdf->Ln(4);
			$pdf->Cell(0,5,'Laporan Permintaan Barang',0,0,'L');
			$pdf->Ln(4);
			$pdf->Cell(0,5,'Periode Tanggal : '.$this->input->post('date_from').' s/d '.$this->input->post('date_to'),0,0,'L');
			$pdf->Ln(10);
			// /* - Header - */
			// $pdf->Cell(40,15,'NOMOR',1,0,'C');
			// $pdf->Cell(30,15,'TANGGAL',1,0,'C');
			// $pdf->Cell(80,15,'TOKO',1,0,'C');
			// $pdf->Cell(40,15,'JUMLAH ITEM',1,0,'C');
			// $pdf->Cell(40,15,'TOTAL',1,0,'C');
			// $pdf->Cell(0,15,'STATUS',1,0,'C');
			// $pdf->Ln(15);
			// $pdf->SetFont('Arial','',8);
			/* - Row - */
			$i=1;
			...
			foreach ($result as $row) {
				...
				$pdf->Cell(20,5,'TOKO',0,0,'L');
				$pdf->Cell(10,5,':',0,0,'C');
				$pdf->Cell(50,5,$row->toko_name,0,0,'L');
				$pdf->Cell(20,5,'STATUS',0,0,'L');
				$pdf->Cell(10,5,':',0,0,'C');
				$pdf->Cell(0,5,$row->status_name,0,0,'L');
				$pdf->Ln(5);
				...
				$pdf->Cell(10,5,':',0,0,'C');
				$pdf->Cell(0,5,number_format($row->total),0,0,'L');
				$pdf->Ln(5);
				$j=1;
				...
				foreach ($result_detail as $rows) {
					...
					$pdf->Cell(30,5,number_format($rows->harga),1,0,'R');
					$pdf->Cell(30,5,number_format($rows->harga*$rows->jumlah),1,0,'R');
